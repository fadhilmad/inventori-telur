        ...
        <div class="row">
            <div class="col-lg-6">
                <div class="card card-primary card-outline">
                    <form class="form-horizontal" method="POST" action="{{ route('master-data.telur.update', $telur->id) }}">
                        @method('PUT')
                        @csrf
                        <div class="card-body">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control @error('name') is-invalid @enderror"
                                        id="inputPassword3" placeholder="Nama" name="name" value="{{ $telur->name }}">
                                    @error('name')
                                    ...
                                    <select name="satuan_besar_id"
                                        ...
                                        @foreach ($satuanBesar->sortBy('name') as $key => $besar)
                                            <option value="{{ $besar->id }}"
                                                {{ $telur->satuan_besar_id == $besar->id ? 'selected' : '' }}>
                                                {{ $besar->name }}</option>
                                        @endforeach
                                        </option>
                                    </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" min="1"
                                        ...
                                        id="inputPassword3" placeholder="Alamat" name="isi_satuan_kecil"
                                        value="{{ $telur->isi_satuan_kecil }}">
                                ...
                                <div class="col-sm-10">
                                    <select name="satuan_kecil_id"
                                        ...
                                        @foreach ($satuanKecil->sortBy('name') as $key => $besar)
                                            <option value="{{ $besar->id }}"
                                                {{ $telur->satuan_kecil_id == $besar->id ? 'selected' : '' }}>
                                                {{ $besar->name }}</option>
                                        @endforeach
                                        </option>
                                    </select>
                        ...
                        <div class="card-footer">
                            <button type="submit" class="btn btn-info" dusk="btn-telur-update">Simpan</button>
                        </div>
                        <!-- /.card-footer -->
                    </form>
                </div>
